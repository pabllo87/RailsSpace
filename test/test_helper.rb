...
class Test::Unit::TestCase
  ...
  def assert_length(boundary, object, attribute, length, options = {})
    ...
    barley_invalid = barley_invalid_string(boundary, length, valid_char)
    
    # tested one outside the boundary
    object[attribute] = barley_invalid
    assert !object.valid?, "#{object[attribute]} (length #{object[attribute].length}) should raise a length error"
    assert_equal correct_error_message(boundary, length), object.errors.on(attribute)
    # tested boundary
    barley_valid = valid_char * length
    object[attribute] = barley_valid
    assert object.valid?, "#{object[attribute]} (length #{object[attribute].length} should be on the boundary of validity)"
  end
  
  # creates an attribute that is only slightly incorrect
  def barley_invalid_string(boundary, length, valid_char)
    if boundary == :max
